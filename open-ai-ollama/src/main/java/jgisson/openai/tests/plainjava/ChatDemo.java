package jgisson.openai.tests.plainjava;
...
public class ChatDemo {
    ...
    public static void main(String[] args) throws Exception {
        ChatRequest chatRequest = new ChatRequest(
                MODEL,
                List.of(new Message("user", "List all the movies directed by Quentin Tarantino")),
                TEMPERATURE);
        ...
        if (response.statusCode() == 200) {
            ...
            System.out.println("\tOpenAI API call error with HTTP status " + response.statusCode());
        }
    }
}
